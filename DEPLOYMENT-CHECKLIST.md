...
**See `PRODUCTION-READINESS.md` first** — it lists everything that must be
configured with your own credentials (VAPID key, TURN server, Sentry DSN,
etc.) before this is safe to launch publicly.

## Already done for you
- Firebase Auth (email/password + Google), Firestore + Storage security
  rules (see `firebase/firestore.rules` and `firebase/storage.rules`),
  Cloudinary uploads
- Feed, Network, Jobs, Articles, Learning, real-time Chat (with reply,
  reactions, edit, delete, read receipts, star, forward, mute, in-chat
  search), Voice/Video calling with call history
...
## File layout (already matches this checklist — nothing to move)
```
project-root/
  firebase.json
  index.html, manifest.json, robots.txt, ... (everything Hosting serves)
  firebase/
    firestore.rules
    storage.rules
  functions/
...
```
(`firebase.json` lives at the project root — the same folder as
`index.html` — and it points at `firebase/firestore.rules` and
`firebase/storage.rules` for the rules files themselves. No `--config`
flag is needed as long as you run this from the project root; adding
`--config firebase/firebase.json` looks for the config file in the wrong
place and will fail.)
