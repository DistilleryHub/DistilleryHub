...
// APP_PATH: set as an environment variable (v2 functions use process.env,
// not the v1 functions.config() API) — e.g. APP_PATH="/" for a root
// deployment. Defaults to the GitHub Pages subfolder this was originally
// deployed under if unset.
const APP_PATH = process.env.APP_PATH || '/DistilleryHub/';

function clickUrlFor(n) {
  if (n.convoId) return `${APP_PATH}?open=chat`;
  if (n.jobId) return `${APP_PATH}?open=jobs:${n.jobId}`;
  return APP_PATH;
}
