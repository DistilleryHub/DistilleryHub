...
## 4. NEW in this update — CallContext.jsx + CallScreen.jsx

Real bug found (not just the environment/permission issue described above):
`startCall` and `joinCall` called `getUserMedia()` with no `try/catch`. If the
browser denied camera/mic permission, the promise rejected uncaught — no call
doc was created (startCall) or the incoming-call screen just sat there
(joinCall) with zero visible feedback. Tapping Accept/Call again repeated the
exact same silent failure, which is why the console showed `NotAllowedError`
over and over.

**Fixed:**
- `CallContext.jsx` — `getUserMedia` in both `startCall` and `joinCall` is now
  wrapped in `try/catch`. On failure it sets a new `callError` string (via
  `describeMediaError()`, which gives a readable message for
  `NotAllowedError` / `NotFoundError` / `NotReadableError`) and returns early
  instead of continuing into a broken call state. `callError` is exposed from
  `useCall()`.
- `CallScreen.jsx` — new `CallErrorBanner` component shows `callError` as a
  dismissible red banner at the top of whichever call screen is active
  (incoming / outgoing / active). The incoming-call screen stays visible on
  failure so the user can retry Accept or tap Decline instead of the app
  looking frozen.

Replace your existing `CallContext.jsx` and `CallScreen.jsx` with the ones in
this zip. No other files changed for this part.
